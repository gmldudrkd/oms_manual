...
> Gentle Monster JP 법인을 IIC OMS 로 통합합니다.  
...
#### ✅ 변경 내용
1. 필터 구조 변경 - 기존에 status 검색으로 구분했지만 신규에서는 Order Status Filter + Fulfillment Status Filter 두 개의 별도 필터로 구분. 
2. Brand / Corp / Channel 컬럼 추가.
...
2. 더 상세한 필드 정보는 [stock-overview](/docs/stock/overview) 문서 참고
...
#### ✅ 기능정의 
- 현재 Available 수량을 채널에 즉시 전송할 수 있는 기능입니다.
- 판매를 잠시 중단하거나 프리오더 종료 후 재고 반영이 필요할 때 사용합니다.
- Available 수량 기준으로 한번에 하나의 채널에만 적용이 가능합니다.
#### ✅ 기능위치
- Stock > Channel Stock Setting 탭 내 'Send Available Qty' 버튼
#### ✅ 설정 가능한 검색 및 제품 조건
- 제품 조건 : 한번에 하나의 채널에만 적용이 가능
#### ✅ 사용방법
1. 선택된 제품의 정보와 현재 사용가능 재고 (Available Qty)를 확인한다.
2. 'Send Available Qty' 버튼을 클릭한다.
3. 클릭한 당시의 Available Qty 정보를 확인한다.
3. ‘save’ 버튼 클릭 시 즉시 해당채널에 재고가 전송된다.
