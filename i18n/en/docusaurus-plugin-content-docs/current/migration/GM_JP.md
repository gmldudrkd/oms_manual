...
# GM_JP Core Operations

> Gentle Monster JP entity is being integrated into IIC OMS.  
...
#### ✅ Changes
1. The filter structure has changed. In the existing system, status search was used to distinguish cases. In the new system, this is separated into two filters: Order Status Filter and Fulfillment Status Filter.
2. Brand, Corp, and Channel columns have been added.
...
:::note
Therefore, **because stock moved within ERP can be received immediately, the Placeholder feature is no longer needed and has been removed, and can be checked through the ERP Update field instead.**
:::
...
### Stock Transfer
#### ✅ Feature Definition
- After end-of-day stock is distributed, use this feature to move the stock each channel needs.
- When needed, move stock from a product's Undistributed Qty to a specific channel.
#### ✅ Feature Location
- Stock > Online Stock Setting tab > 'Stock Transfer' button
#### ✅ Applicable Search and Product Conditions
- Search condition: When the selected product has only one channel.
- Product condition: When the **Channel Send Status of the selected products is all 'ON'**.
#### ✅ How to Use
1. Check the selected product's information, its current Available Qty, and Undisitributed Qty.
2. In Transfer Stock, enter the **additional stock quantity** to send for each product.
    - Example of the transferred stock value ⇒ `Available Stock : 1 / Transfer Stock : 4 → Transferred Stock : 5`
    - If the entered quantity exceeds the selected product's Undisitributed Stock by even one, the transfer is not possible.
    - **Entering negative stock (stock deduction) is not possible.**
3. When the 'save' button is clicked, the moved stock behaves as follows:
    1. If unallocated stock exists, it is allocated.
    2. The remaining stock is sent to the channel **immediately**.

### Immediately Send Stock Quantity to Channel

#### ✅ Feature Definition
- This feature immediately sends the current Available quantity to a channel.
- Use it when you need to temporarily suspend sales or reflect stock after a pre-order ends.
...
#### ✅ Applicable Search and Product Conditions
- Product condition: Can be applied to only one channel at a time.
#### ✅ How to Use
1. Check the selected product's information and current Available Qty.
2. Click the 'Send Available Qty' button.
3. Check the Available Qty information at the time of clicking.
3. When the 'save' button is clicked, the stock is immediately sent to the channel.

![Immediately Send Stock Quantity to Channel](/img/send_available_qty.png)